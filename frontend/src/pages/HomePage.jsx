...
const HomePage = () => {
	...
	const prevSlide = () => {
		setCurrentSlide(
			(prev) =>
				(prev - 1 + Math.min(trending.length, 5)) % Math.min(trending.length, 5)
		);
	};
	...
	return (
		<div
			className="min-h-screen bg-[#050505] pb-20 md:pb-0"
									...
									<motion.div
										...
										<div className="flex items-center gap-4 pt-4">
											<button
												onClick={() =>
													navigate(
														`/watch/${featuredMovie.media_type || "movie"}/${
															featuredMovie.id
														}`
													)
												}
			...
			<div className="max-w-7xl mx-auto px-6 md:px-12 mt-16 space-y-12">
				...
				<MovieRow
					...
					onAddToList={addToList}
				/>
				<MovieRow title="Top Rated" movies={topRated} onAddToList={addToList} />
				<MovieRow
